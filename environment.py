...
class TerritoryGameEnvironment:
    def __init__(self, trajectorySaveFileName = None,trajectoryTrackingFileName = None, mapName = 'blank 10 12', bot_infos = [('bot'),('spiral'),('spiral') ]):
        ...
        if self.trajectoryTrackingFileName:
            ...
            for i in range(num_other_players):
                algo = bot_infos[i]
                bot_start_location = start_locations[i+1]
                x_bot,y_bot,color_bot = bot_start_location[0],bot_start_location[1], bot_colors[i]
                bot = None
                if algo == 'spiral':
